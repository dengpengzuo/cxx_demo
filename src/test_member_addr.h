...
namespace Test_Member_Addr {
    ...
    void testPoint3d() {
        Point3D *addr = new Point3D(1, 2, 3);
        {
            ...
            std::cout << "Point3d instance " << addr << " Point3D._x mem address:" << (void *) (&ptr->_x) << std::endl;
            std::cout << "Point3d instance " << addr << " Point3D._z mem address:" << (void *) (&ptr->_z) << std::endl;
        }
        {
    ...
    void testWXYZ() {
        WXYZ *addr = new WXYZ(10, 1, 2, 3);
        {
            ...
            // 无virtual 函数情况下: ptr 和 x 是同一个内存地址.
            // 有virtual 函数情况下：ptr 指向是 vtbl ,vtbl +　8 才是 x　的地址.
            std::cout << "WXYZ instance " << addr << "                  mem address:" << ptr << std::endl;
            std::cout << "WXYZ instance " << addr << " WXYZ::Point3D._x mem address:" << (void *) (&ptr->_x) << std::endl;
            std::cout << "WXYZ instance " << addr << " WXYZ::Point3D._y mem address:" << (void *) (&ptr->_y) << std::endl;
            std::cout << "WXYZ instance " << addr << " WXYZ::Point3D._z mem address:" << (void *) (&ptr->_z) << std::endl;
            std::cout << "WXYZ instance " << addr << " WXYZ._w          mem address:" << (void *) (&ptr->_w) << std::endl;
        }
        {
