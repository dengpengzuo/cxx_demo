...
void printSize() {
  struct PrintInfo info[] = {{"sizeof(int)", sizeof(int)},
                             ...
                             {"sizeof(Y3)", sizeof(Y3)},
                             {"sizeof(Y4)", sizeof(Y4)}};
  // {"sizeof(void)", sizeof(void)}, // TODO: g++支持有警告, clang不支持,
  // sizeof(void)
  for (size_t i = 0; i < sizeof(info) / sizeof(info[0]); ++i) {
    PrintInfo *p = &info[i];
    std::cout << std::left << std::setw(16) << p->name << " := " << std::right
              << std::setw(2) << p->size << std::endl;
  }
}

void test() {
  uint64_t array[] = {0x0102030405060708, 0x0102030405060708};
  std::cout << "   array[0] " << &array[0] << ", array[1] " << &array[1]
            << std::endl;
  {
